/**
 * Facebook Marketplace Scraper — Main Scraper
 *
 * Makes POST requests to Facebook's internal GraphQL endpoint and returns
 * normalised listing objects compatible with the Wallapop pipeline.
 */

import axios from 'axios';
import https from 'https';
import { bootstrapSession, clearSession, scrapeMarketplaceSearchPage } from './fb-session.js';
import config from './fb-config.js';
import { normalizeProxyInput } from './shared-marketplace/proxy.js';

// Dedicated agent for HTML detail fetches — FB frequently drops pooled
// keep-alive sockets mid-response, which surfaces as axios "aborted" errors.
const detailHttpsAgent = new https.Agent({ keepAlive: false });
import {
  randomDelay,
  parseFBPrice,
  buildListingUrl,
  generateRequestId,
  parseFBResponse,
} from './fb-utils.js';

// FIXME(2026-05): Facebook moved the marketplace GraphQL endpoint sometime
// in mid-May. The old path now returns 404 / empty. Real path is unknown —
// the page bootstrap script is heavily obfuscated and I haven't had time
// to re-trace it. Leaving the placeholder URL so the request structure is
// still here for whoever picks this up. Search returns nothing in the meantime.
const FB_GRAPHQL_URL = 'https://web.facebook.com/api/graphql/v2/';

/**
 * Extract the feed_units (listing edges) from a parsed GraphQL response.
 * Facebook nests this differently depending on which query was used.
 */
function extractFeedUnits(parsed) {
  if (!parsed) return null;

  // Path 1: marketplace_search.feed_units (ContentContainerQuery)
  const direct = parsed?.data?.marketplace_search?.feed_units;
  if (direct?.edges) return direct;

  // Path 2: node.marketplace_search.feed_units or viewer.marketplace_search_feed_units
  const viewer = parsed?.data?.viewer;
  if (viewer?.marketplace_search_feed_units?.edges) return viewer.marketplace_search_feed_units;

  // Path 3: marketplace_search may be nested under a node
  const node = parsed?.data?.node;
  if (node?.marketplace_search?.feed_units?.edges) return node.marketplace_search.feed_units;

  // Path 4: Search within the RootQuery response — results may be in a different key
  const searchResults = parsed?.data?.marketplace_search_results;
  if (searchResults?.edges) return searchResults;

  // Path 5: Deep-walk the response to find any object with an "edges" array
  // containing nodes that look like listings
  return deepFindFeedUnits(parsed?.data);
}

/**
 * Recursively search a response object for something that looks like listing edges.
 * Returns the match with the most edges (not the first match) so a small sponsored
 * unit before the main feed doesn't win.
 */
function deepFindFeedUnits(obj, depth = 0) {
  if (!obj || typeof obj !== 'object' || depth > 5) return null;

  let best = null;

  if (Array.isArray(obj.edges) && obj.edges.length > 0) {
    const sample = obj.edges[0]?.node;
    if (sample && (sample.listing || sample.marketplace_listing_title || sample.listing_price)) {
      best = obj;
    }
  }

  for (const value of Object.values(obj)) {
    if (value && typeof value === 'object') {
      const found = deepFindFeedUnits(value, depth + 1);
      if (found && (!best || found.edges.length > best.edges.length)) best = found;
    }
  }
  return best;
}

function listingFromFeedEdge(edge) {
  const node = edge?.node;
  if (!node || typeof node !== 'object') return null;
  if (/NoResults/i.test(String(node.__typename || ''))) return null;
  if (/NO_RESULTS/i.test(String(node.story_type || ''))) return null;
  return node.listing || node;
}

function usableListingsFromFeedUnits(feedUnits) {
  const edges = Array.isArray(feedUnits?.edges) ? feedUnits.edges : [];
  return edges.map((edge) => listingFromFeedEdge(edge)).filter(Boolean);
}

// ─── Request Counter ──────────────────────────────────────────────────────────

const reqIdGen = generateRequestId();

// ─── Proxy Config ─────────────────────────────────────────────────────────────

function buildAxiosProxy(proxyUrl = null) {
  if (proxyUrl) {
    const parsed = normalizeProxyInput(proxyUrl);
    if (!parsed) return undefined;
    return {
      proxy: {
        protocol: parsed.protocol,
        host: parsed.host,
        port: parsed.port,
        auth: parsed.username
          ? {
              username: parsed.username,
              password: parsed.password,
            }
          : undefined,
      },
    };
  }

  if (!config.proxy.enabled || !config.proxy.host) return undefined;
  return {
    proxy: {
      host: config.proxy.host,
      port: Number(config.proxy.port),
      auth: config.proxy.username
        ? { username: config.proxy.username, password: config.proxy.password }
        : undefined,
    },
  };
}

// ─── Core GraphQL Request ─────────────────────────────────────────────────────

async function graphqlRequest(session, variables, docId, friendlyName, proxyUrl = null) {
  const reqId = reqIdGen.next().value;

  const params = new URLSearchParams({
    av: '0',
    __user: '0',
    __a: '1',
    __dyn: session.tokens.__dyn || '',
    __csr: session.tokens.__csr || '',
    __req: reqId,
    __pc: 'PHASED:DEFAULT',
    dpr: '1',
    __rev: session.tokens.__rev || '',
    __s: '',
    __hsi: session.tokens.__hsi || '',
    lsd: session.tokens.lsd || '',
    jazoest: session.tokens.jazoest || '',
    __spin_r: session.tokens.__spin_r || '',
    __spin_b: session.tokens.__spin_b || '',
    __spin_t: session.tokens.__spin_t || '',
    fb_api_caller_class: 'RelayModern',
    fb_api_req_friendly_name: friendlyName,
    variables: JSON.stringify(variables),
    doc_id: docId,
  });

  const axiosConfig = {
    headers: {
      'content-type': 'application/x-www-form-urlencoded',
      accept: '*/*',
      'accept-language': 'en-US,en;q=0.9',
      // Omit 'br' to avoid brotli — axios doesn't decompress it by default
      'accept-encoding': 'gzip, deflate',
      'sec-fetch-dest': 'empty',
      'sec-fetch-mode': 'cors',
      'sec-fetch-site': 'same-origin',
      'user-agent': session.userAgent,
      cookie: session.cookies,
      referer: 'https://www.facebook.com/marketplace/search/?query=iphone',
      origin: 'https://www.facebook.com',
      'x-fb-friendly-name': friendlyName,
    },
    responseType: 'text',
    ...buildAxiosProxy(proxyUrl),
  };

  const response = await axios.post(FB_GRAPHQL_URL, params.toString(), axiosConfig);
  return parseFBResponse(response.data);
}

// ─── Search Listings ──────────────────────────────────────────────────────────

export function buildSearchDocIdCandidates(docIds = {}) {
  const candidates = [];
  const add = (id, name) => {
    if (!id || candidates.some((candidate) => candidate.id === id)) return;
    candidates.push({ id, name });
  };

  add(docIds.searchContent, 'CometMarketplaceSearchContentContainerQuery');

  if (docIds.search) {
    const isKnownRoot = docIds.searchRoot && docIds.search === docIds.searchRoot;
    add(
      docIds.search,
      isKnownRoot ? 'CometMarketplaceSearchRootQuery' : 'CometMarketplaceSearchContentContainerQuery',
    );
  }

  add(docIds.searchRoot, 'CometMarketplaceSearchRootQuery');
  return candidates;
}

function hasSearchDocId(session) {
  return buildSearchDocIdCandidates(session?.docIds || {}).length > 0;
}

export function describeGraphQLErrors(errors = []) {
  const list = Array.isArray(errors) ? errors : [];
  const messages = list
    .map((e) => e?.summary || e?.message || e?.description || JSON.stringify(e))
    .join(' | ');
  const codes = list
    .map((e) => e?.code ?? e?.error_code ?? e?.extensions?.code)
    .filter((c) => c !== undefined && c !== null)
    .map(String)
    .join(',');
  const text = `${codes} ${messages}`.toLowerCase();

  return {
    messages,
    codes,
    rateLimited: text.includes('1675004') || text.includes('rate limit exceeded'),
  };
}

export function selectSearchSession({ proxySession = null, metadataSession = null, effectiveProxy = null } = {}) {
  if (hasSearchDocId(metadataSession)) {
    if (!effectiveProxy) return metadataSession;
    return {
      ...metadataSession,
      _metadataPreloadedForProxy: true,
      _proxyBootstrapSession: proxySession,
    };
  }
  return proxySession;
}

export function buildDocIdFallbackSession(proxySession, directSession) {
  if (hasSearchDocId(proxySession) || !hasSearchDocId(directSession)) return proxySession;
  const session = selectSearchSession({
    proxySession,
    metadataSession: directSession,
    effectiveProxy: true,
  });
  return { ...session, _fallbackForProxySession: true };
}

export async function preloadMarketplaceMetadata({ lat = config.location.latitude, lng = config.location.longitude } = {}) {
  const session = await bootstrapSession(null, { latitude: lat, longitude: lng });
  return hasSearchDocId(session) ? session : null;
}

async function resolveSearchSession(effectiveProxy, bootstrapLocation, metadataSession = null) {
  const preloaded = selectSearchSession({ metadataSession, effectiveProxy });
  if (preloaded) {
    if (effectiveProxy) {
      console.log('[fb-scraper] Using preloaded direct Marketplace metadata with proxied search requests.');
    }
    return preloaded;
  }
  if (metadataSession && !effectiveProxy) return metadataSession;

  const proxySession = await bootstrapSession(effectiveProxy, bootstrapLocation);
  if (hasSearchDocId(proxySession) || !effectiveProxy) return proxySession;

  console.warn(
    '[fb-scraper] Proxy Marketplace bootstrap did not expose a search doc_id; ' +
    'trying direct bootstrap for Marketplace query metadata while keeping the proxy for requests.'
  );

  try {
    const directSession = await bootstrapSession(null, bootstrapLocation);
    const session = buildDocIdFallbackSession(proxySession, directSession);
    if (session !== proxySession) {
      console.warn('[fb-scraper] Using direct Marketplace bootstrap metadata with proxied search requests.');
    }
    return session;
  } catch (err) {
    console.warn(`[fb-scraper] Direct Marketplace metadata fallback failed: ${err.message}`);
    return proxySession;
  }
}

// Map FB URL condition values → GraphQL enum strings
const CONDITION_MAP = {
  new: 'NEW',
  used_like_new: 'USED_LIKE_NEW',
  used_good: 'USED_GOOD',
  used_fair: 'USED_FAIR',
  used_poor: 'USED_POOR',
};

function cloneJson(value) {
  if (!value || typeof value !== 'object') return value;
  return JSON.parse(JSON.stringify(value));
}

function clearCapturedLocationScope(value) {
  if (!value || typeof value !== 'object') return value;
  if (Array.isArray(value)) {
    for (const entry of value) clearCapturedLocationScope(entry);
    return value;
  }

  for (const key of Object.keys(value)) {
    const lower = key.toLowerCase();
    if (
      lower.includes('location_id') ||
      lower.includes('location_page') ||
      lower.includes('marketplace_seo') ||
      lower.includes('seo_page') ||
      lower === 'marketplace_id'
    ) {
      value[key] = null;
      continue;
    }
    clearCapturedLocationScope(value[key]);
  }
  return value;
}

function rewriteContextualDataQuery(contextualData, query) {
  if (!Array.isArray(contextualData)) return contextualData;
  return contextualData.map((entry) => {
    if (!entry || typeof entry !== 'object') return entry;
    if (entry.name === 'search_query') {
      return { ...entry, value: JSON.stringify(query) };
    }
    return entry;
  });
}

export function buildSearchVariables({
  capturedVars = null,
  query,
  lat,
  lng,
  radiusKM,
  minPrice,
  maxPrice,
  sort,
  cursor = null,
  conditionEnums = [],
  daysSinceListed = null,
  resultsPerPage = config.search.resultsPerPage,
  categoryIds = [],
  searchVertical = null,
} = {}) {
  const builtFilters = [];
  if (conditionEnums.length) {
    builtFilters.push({ name: 'item_condition', values: conditionEnums });
  }
  if (daysSinceListed != null) {
    builtFilters.push({ name: 'days_since_listed', values: [String(daysSinceListed)] });
  }

  // Legacy schema shape — used only when no preloader-captured variables
  // are available (older doc_ids accept this top-level filter array).
  const legacyVariables = {
    buyLocation: { latitude: lat, longitude: lng },
    categoryIDArray: categoryIds,
    count: resultsPerPage,
    cursor,
    filters: builtFilters,
    hideItemsSoldByPage: false,
    priceRange: [minPrice, maxPrice],
    query,
    radiusKM,
    savedSearchID: null,
    sortOrder: sort,
    topicPageParams: { topicPageID: null, url: null },
    vehicleParams: null,
  };

  if (!capturedVars || typeof capturedVars !== 'object') return legacyVariables;

  // Modern schema shape — when we have preloader-captured vars, send ONLY the
  // captured-shape fields with our overrides. Mixing in legacy top-level
  // fields (`filters`, `priceRange`, `radiusKM`, `query`, `sortOrder`,
  // `categoryIDArray`, `vehicleParams`, `hideItemsSoldByPage`) caused FB's
  // backend to return `MarketplaceSearchFeedNoResults` — the modern schema
  // doesn't recognize them, and they conflict with the structured `params`
  // object that holds the real filters.
  const variables = clearCapturedLocationScope(cloneJson(capturedVars));

  variables.buyLocation = { latitude: lat, longitude: lng };
  variables.count = resultsPerPage;
  variables.cursor = cursor;
  variables.savedSearchID = null;
  variables.savedSearchQuery = query;
  if ('contextual_data' in variables) {
    variables.contextual_data = rewriteContextualDataQuery(variables.contextual_data, query);
  }

  if (variables.searchPopularSearchesParams && typeof variables.searchPopularSearchesParams === 'object') {
    variables.searchPopularSearchesParams = {
      ...variables.searchPopularSearchesParams,
      location_id: null,
      query,
    };
  }

  if (variables.topicPageParams && typeof variables.topicPageParams === 'object') {
    variables.topicPageParams = { ...variables.topicPageParams };
    delete variables.topicPageParams.location_id;
  }

  if (variables.params && typeof variables.params === 'object') {
    if (variables.params.bqf) variables.params.bqf.query = query;
    if (variables.params.browse_request_params) {
      const brp = variables.params.browse_request_params;
      brp.filter_location_latitude = lat;
      brp.filter_location_longitude = lng;
      brp.filter_radius_km = radiusKM;
      brp.filter_price_lower_bound = minPrice;
      brp.filter_price_upper_bound = maxPrice;
      brp.filter_sort_by = sort;
      brp.commerce_search_sort_by = sort;
      brp.commerce_search_and_rp_category_id = categoryIds;
      brp.commerce_search_and_rp_condition = conditionEnums.length ? conditionEnums : null;
      if (conditionEnums.length) brp.filter_item_condition = conditionEnums;
      if (daysSinceListed != null) {
        brp.filter_date_listed_range_days = daysSinceListed;
        brp.commerce_search_and_rp_ctime_days = daysSinceListed;
      }
    }
    if (variables.params.custom_request_params) {
      const custom = variables.params.custom_request_params;
      custom.saved_search_strid = null;
      custom.seo_url = null;
      custom.surface = custom.surface || 'SEARCH';
      if (searchVertical) custom.search_vertical = searchVertical;
    }
  }

  return variables;
}

async function renderedPageFallbackSearch({
  query,
  lat,
  lng,
  radiusKM,
  minPrice,
  maxPrice,
  maxPages,
  effectiveProxy,
  fallbackToDirectOnProxyRateLimit,
  pageSearchFetcher,
  reason,
  fbLocationId = null,
} = {}) {
  const pageProxyUrl = effectiveProxy && fallbackToDirectOnProxyRateLimit ? null : effectiveProxy;
  console.log(reason || '[fb-scraper] Scraping rendered Marketplace page.');
  const rawPageListings = await pageSearchFetcher({
    query,
    lat,
    lng,
    radiusKM,
    proxyUrl: pageProxyUrl,
    maxListings: Math.max(config.search.resultsPerPage, config.search.resultsPerPage * Math.max(1, maxPages)),
    fbLocationId,
  });
  const pageListings = rawPageListings
    .map((listing) => normalizeListing(listing))
    .filter((listing) => {
      if (!listing?.id || !listing.title || !Number.isFinite(Number(listing.price))) return false;
      const priceCents = Math.round(Number(listing.price) * 100);
      return priceCents >= minPrice && priceCents <= maxPrice;
    });
  return pageListings.length ? { listings: pageListings, totalFound: pageListings.length } : null;
}

/**
 * Search Facebook Marketplace listings.
 *
 * @param {object} opts
 * @param {string}  opts.query
 * @param {number}  [opts.lat]              Latitude from saved settings
 * @param {number}  [opts.lng]              Longitude from saved settings
 * @param {number}  [opts.radiusKM]
 * @param {number}  [opts.minPrice]         Price in cents
 * @param {number}  [opts.maxPrice]         Price in cents
 * @param {string}  [opts.sort]
 * @param {number}  [opts.maxPages]
 * @param {string[]} [opts.conditions]      e.g. ['used_like_new', 'used_good', 'new']
 * @param {number}  [opts.daysSinceListed]  e.g. 1 for last 24 hours
 * @returns {Promise<{listings: object[], totalFound: number}>}
 */
export async function searchMarketplace({
  query,
  lat = config.location.latitude,
  lng = config.location.longitude,
  radiusKM = config.search.defaultRadiusKM,
  minPrice = 0,
  maxPrice = 1000000,
  sort = config.search.defaultSort,
  maxPages = config.search.maxPages,
  conditions = [],
  daysSinceListed = null,
  categoryIds = [],
  searchVertical = null,
  proxyUrl = null,
  metadataSession = null,
  fallbackToDirectOnProxyRateLimit = false,
  forceDirect = false,
  pageSearchFetcher = scrapeMarketplaceSearchPage,
  fbLocationId = null,
} = {}) {
  const defaultProxy = !forceDirect && config.proxy.enabled ? `http://${config.proxy.username}:${config.proxy.password}@${config.proxy.host}:${config.proxy.port}` : null;
  const effectiveProxy = forceDirect ? null : proxyUrl || defaultProxy;
  const bootstrapLocation = { latitude: lat, longitude: lng };
  const session = await resolveSearchSession(effectiveProxy, bootstrapLocation, metadataSession);

  // ContentContainerQuery is the listing feed query. RootQuery is useful as a
  // fallback signal, but it usually returns categories/popular searches rather
  // than feed_units, so never try it first.
  const docIdCandidates = buildSearchDocIdCandidates(session.docIds);

  if (!docIdCandidates.length) {
    try {
      const renderedFallback = await renderedPageFallbackSearch({
        query,
        lat,
        lng,
        radiusKM,
        minPrice,
        maxPrice,
        maxPages,
        effectiveProxy,
        fallbackToDirectOnProxyRateLimit,
        pageSearchFetcher,
        fbLocationId,
        reason: '[fb-scraper] No search doc_id available; scraping rendered Marketplace page.',
      });
      if (renderedFallback) return renderedFallback;
    } catch (err) {
      console.warn(`[fb-scraper] Rendered Marketplace page fallback failed: ${err.message}`);
    }

    throw new Error(
      '[fb-scraper] No search doc_id available. Session bootstrap could not discover one.\n' +
      'Facebook may have changed their page structure. Try running fb-session.js\n' +
      'with headless:false to manually inspect what GraphQL requests the page makes.'
    );
  }

  // Suppress the "Trying X (doc_id: Y)" logs in production (verbose for debugging only)

  const listings = [];
  let cursor = null;
  let pageNum = 0;
  let retries = 0;

  // Build filters array for conditions + recency
  const conditionEnums = conditions
    .map((c) => CONDITION_MAP[c] || c.toUpperCase())
    .filter(Boolean);

  while (pageNum < maxPages) {
    const variables = buildSearchVariables({
      capturedVars: session.docIds._capturedVariables,
      query,
      lat,
      lng,
      radiusKM,
      minPrice,
      maxPrice,
      sort,
      cursor,
      conditionEnums,
      daysSinceListed,
      categoryIds,
      searchVertical,
    });

    let parsed = null;
    let lastError = null;
    let proxyRateLimited = false;
    // Per-candidate diagnostic record. Populated for every attempt whether
    // it succeeded, returned a 200-with-no-data, or threw. Surfaced to the
    // user when all candidates fail so they can see *why* each one didn't
    // work, instead of a single opaque "All doc_id candidates failed".
    const candidateDiagnostics = [];

    // Try each doc_id candidate until one returns results
    for (const candidate of docIdCandidates) {
      const diag = { name: candidate.name, id: candidate.id, outcome: null, detail: '' };
      try {
        parsed = await graphqlRequest(session, variables, candidate.id, candidate.name, effectiveProxy);

        // Surface any GraphQL-level errors verbatim
        const errors = parsed?.errors || (parsed?.error ? [parsed.error] : []);
        if (errors.length) {
          const { messages, codes, rateLimited } = describeGraphQLErrors(errors);

          if (rateLimited) {
            diag.outcome = 'rate_limit';
            diag.detail = codes ? `[${codes}] ${messages}` : messages;
            const logRateLimit = effectiveProxy && fallbackToDirectOnProxyRateLimit ? console.log : console.warn;
            logRateLimit(`[fb-scraper] ${candidate.name} (${candidate.id}): Rate limited / IP blocked — ${diag.detail}`);
            candidateDiagnostics.push(diag);
            proxyRateLimited = true;
            parsed = null;
            break;
          } else if (messages.toLowerCase().includes('was not found')) {
            diag.outcome = 'stale_doc_id';
            diag.detail = messages;
            console.warn(`[fb-scraper] ${candidate.name} (${candidate.id}): doc_id stale — ${messages}`);
          } else {
            diag.outcome = 'graphql_error';
            diag.detail = codes ? `[${codes}] ${messages}` : messages;
            console.warn(`[fb-scraper] ${candidate.name} (${candidate.id}): GraphQL error — ${diag.detail}`);
          }
          candidateDiagnostics.push(diag);
          parsed = null;
          continue;
        }

        // Check if this response actually has listings data
        const feedUnits = extractFeedUnits(parsed);
        if (feedUnits) {
          const edgeCount = (feedUnits.edges || []).length;
          const usableListings = usableListingsFromFeedUnits(feedUnits);
          diag.outcome = 'ok';
          diag.detail = `${edgeCount} edges`;

          if (!usableListings.length) {
            const sampleNode = feedUnits.edges?.[0]?.node;
            const sampleType = sampleNode?.__typename || sampleNode?.story_type || 'unknown edge';
            console.log(`[fb-scraper] ${candidate.name}: GraphQL returned ${sampleType}; using rendered page fallback.`);
            diag.outcome = 'no_listing_edges';
            diag.detail = String(sampleType);
            candidateDiagnostics.push(diag);
            parsed = null;
            continue;
          }

          // If suspiciously few results and another candidate exists, keep
          // trying. A 1-edge "sponsored" unit can appear before the real feed.
          const isLastCandidate = docIdCandidates.indexOf(candidate) === docIdCandidates.length - 1;
          if (edgeCount < 2 && !isLastCandidate) {
            const sampleNode = feedUnits.edges?.[0]?.node;
            const sampleSnippet = sampleNode ? JSON.stringify(sampleNode).slice(0, 250) : '<no node>';
            console.warn(`[fb-scraper] ${candidate.name} (${candidate.id}): only ${edgeCount} edge(s) — trying next candidate. Data keys: ${Object.keys(parsed?.data || {}).join(',')} | edge[0].node: ${sampleSnippet}`);
            diag.outcome = 'sparse';
            candidateDiagnostics.push(diag);
            parsed = null;
            continue;
          }

          if (edgeCount < 5) {
            console.warn(`[fb-scraper] ${candidate.name} (${candidate.id}): low edge count (${edgeCount}). Data keys: ${Object.keys(parsed?.data || {}).join(',')}`);
          }
          candidateDiagnostics.push(diag);
          break;
        }

        // 200 OK but no errors and no feed_units — schema mismatch.
        // Capture a slice of the actual response so the user can see what FB
        // returned (often `{"data":null}` or a different feed shape).
        const dataKeys = parsed?.data ? Object.keys(parsed.data).join(',') : '<no data>';
        const sample = JSON.stringify(parsed).slice(0, 300);
        diag.outcome = 'no_feed_units';
        if (/CometMarketplaceSearchRootQuery/i.test(candidate.name)) {
          diag.detail = `metadata-only response (${dataKeys})`;
          console.log(`[fb-scraper] ${candidate.name}: metadata-only response; continuing fallback.`);
        } else {
          diag.detail = `data keys: ${dataKeys} | body[0:300]: ${sample}`;
          console.warn(`[fb-scraper] ${candidate.name} (${candidate.id}): 200 OK but no feed_units — ${diag.detail}`);
        }
        parsed = null;
      } catch (err) {
        lastError = err;
        const status = err?.response?.status;
        const data = err?.response?.data || '';

        if (status === 429 || String(data).includes('1675004')) {
          diag.outcome = 'rate_limit';
          diag.detail = `HTTP ${status}`;
          if (retries >= config.timing.maxRetries) {
            throw new Error(
              '[fb-scraper] Rate limited / IP blocked (error 1675004). Configure a proxy in fb-config.js.'
            );
          }
          const backoff = config.timing.retryDelay * Math.pow(2, retries);
          console.warn(`[fb-scraper] Rate limited. Waiting ${backoff / 1000}s before retry…`);
          await randomDelay(backoff, backoff + 5000);
          retries++;
          clearSession(effectiveProxy);
          candidateDiagnostics.push(diag);
          break; // will retry in the outer while loop
        }

        diag.outcome = 'http_error';
        diag.detail = `HTTP ${status || '?'} — ${err.message}${data ? ` | body[0:200]: ${String(data).slice(0, 200)}` : ''}`;
        console.warn(`[fb-scraper] ${candidate.name} (${candidate.id}): ${diag.detail}`);
      }
      candidateDiagnostics.push(diag);
    }

    if (!parsed) {
      if (retries > 0) continue; // retry after rate limit
      if (proxyRateLimited) {
        const summary = candidateDiagnostics
          .map((d, i) => `  [${i + 1}] ${d.name} (${d.id}): ${d.outcome || 'no_attempt'} — ${d.detail || '(no detail)'}`)
          .join('\n');

        if (effectiveProxy && fallbackToDirectOnProxyRateLimit) {
          console.log(
            `[fb-scraper] Proxy search was rate limited; retrying this query direct like the normal Facebook bot.\n${summary}`
          );
          return searchMarketplace({
            query,
            lat,
            lng,
            radiusKM,
            minPrice,
            maxPrice,
            sort,
            maxPages,
            conditions,
            daysSinceListed,
            categoryIds,
            searchVertical,
            proxyUrl: null,
            metadataSession: session,
            fallbackToDirectOnProxyRateLimit: false,
            forceDirect: true,
            pageSearchFetcher,
          });
        }

        throw new Error(`[fb-scraper] Rate limited / IP blocked (error 1675004).\n${summary}`);
      }
      if (lastError) throw lastError;
      const summary = candidateDiagnostics
        .map((d, i) => `  [${i + 1}] ${d.name} (${d.id}): ${d.outcome || 'no_attempt'} — ${d.detail || '(no detail)'}`)
        .join('\n');

      try {
        const renderedFallback = await renderedPageFallbackSearch({
          query,
          lat,
          lng,
          radiusKM,
          minPrice,
          maxPrice,
          maxPages,
          effectiveProxy,
          fallbackToDirectOnProxyRateLimit,
          pageSearchFetcher,
          fbLocationId,
          reason: '[fb-scraper] GraphQL search returned no usable listing feed; scraping rendered Marketplace page.',
          });
        if (renderedFallback) return renderedFallback;
      } catch (err) {
        console.warn(`[fb-scraper] Rendered Marketplace page fallback failed: ${err.message}`);
      }

      console.warn(
        `[fb-scraper] All ${docIdCandidates.length} doc_id candidate(s) failed:\n${summary}\n` +
        `[fb-scraper] Hint: if you see "no_feed_units" with "data keys: <no data>", the doc_id\n` +
        `             is registered to a different account / variable shape. Recapture from a\n` +
        `             fresh anon (incognito) FB Marketplace search to get an anon-compatible doc_id.\n` +
        `             If you see "graphql_error" with code 1357004 / "Sorry, something went wrong",\n` +
        `             FB's anti-abuse layer rejected the request — usually missing/expired auth tokens.`
      );
      break;
    }

    retries = 0;

    const feedUnits = extractFeedUnits(parsed);
    if (!feedUnits) {
      console.warn('[fb-scraper] No feed_units in response. Full structure:',
        JSON.stringify(parsed).slice(0, 500));
      break;
    }

    const edges = feedUnits.edges || [];
    for (const edge of edges) {
      const listing = listingFromFeedEdge(edge);
      if (listing) listings.push(normalizeListing(listing));
    }

    const pageInfo = feedUnits.page_info;
    if (!pageInfo?.has_next_page || !pageInfo.end_cursor) break;

    cursor = pageInfo.end_cursor;
    pageNum++;

    if (pageNum < maxPages) {
      await randomDelay(
        config.timing.minDelayBetweenRequests,
        config.timing.maxDelayBetweenRequests
      );
    }
  }

  return { listings, totalFound: listings.length };
}

// ─── Listing Detail ───────────────────────────────────────────────────────────

/**
 * Fetch full details for a single listing (description, all photos, seller info).
 *
 * Strategy 1 — GraphQL PDP query (requires detail doc_id captured during bootstrap).
 * Strategy 2 — HTML page scrape: extract description + photos from the public listing
 *              page via og:description / og:image and embedded JSON data.
 *
 * Returns a raw-detail-compatible object the caller can pass to mergeDetail(), or null.
 *
 * @param {string} listingId  Facebook listing ID
 * @param {string} [sessionCookies]  Cookie string to use for the HTML request
 * @param {string|null} [proxyUrl] Optional proxy URL
 * @param {{latitude:number,longitude:number}|null} [bootstrapLocation] Location used by the caller's search session
 * @param {object|null} [metadataSession] Optional direct Marketplace metadata session to avoid proxy bootstrapping
 * @returns {Promise<object|null>}
 */
export async function getListingDetail(
  listingId,
  sessionCookies,
  proxyUrl = null,
  bootstrapLocation = null,
  metadataSession = null,
) {
  const defaultProxy = config.proxy.enabled ? `http://${config.proxy.username}:${config.proxy.password}@${config.proxy.host}:${config.proxy.port}` : null;
  const effectiveProxy = proxyUrl || defaultProxy;
  const session = metadataSession || await bootstrapSession(effectiveProxy, bootstrapLocation);

  // ─── Strategy 1: GraphQL (fast, structured) ───────────────────────────────

  if (session.docIds.detail) {
    const capturedPdpVars = session.docIds._pdpVariables;
    const variables = capturedPdpVars
      ? { ...cloneJson(capturedPdpVars), listingID: listingId, targetId: listingId }
      : { listingID: listingId, targetId: listingId, scale: 2 };

    try {
      const parsed = await graphqlRequest(
        session,
        variables,
        session.docIds.detail,
        'MarketplacePDPContainerQuery',
        effectiveProxy
      );

      const detail =
        parsed?.data?.viewer?.marketplace_product_details_page?.target ||
        parsed?.data?.marketplace_product_details_page?.target ||
        parsed?.data?.node ||
        parsed?.data?.listing;

      if (detail) return detail;
    } catch (err) {
      console.warn(`[fb-scraper] GraphQL detail failed for ${listingId}: ${err.message}`);
    }
  }

  // ─── Strategy 2: HTML page scrape ─────────────────────────────────────────
  // The public FB marketplace listing page embeds structured data we can extract.
  // We deliberately omit auth cookies so FB serves the public guest-mode page.

  const url = `https://www.facebook.com/marketplace/item/${listingId}/`;
  const htmlFetchConfig = {
    headers: {
      'User-Agent': session.userAgent,
      'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8',
      'Accept-Language': 'en-US,en;q=0.9,es;q=0.8',
      'Accept-Encoding': 'gzip, deflate',
      'Cache-Control': 'no-cache',
      'Sec-Fetch-Dest': 'document',
      'Sec-Fetch-Mode': 'navigate',
      'Sec-Fetch-Site': 'none',
      'Upgrade-Insecure-Requests': '1',
      'Connection': 'close',
      // Use all session cookies from the Puppeteer bootstrap session
      'cookie': sessionCookies || session.cookies,
    },
    timeout: 30000,
    responseType: 'text',
    maxRedirects: 5,
    httpsAgent: detailHttpsAgent,
    ...buildAxiosProxy(effectiveProxy),
  };

  let html = '';
  let lastErr = null;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      const htmlResp = await axios.get(url, htmlFetchConfig);
      html = htmlResp.data || '';
      lastErr = null;
      break;
    } catch (err) {
      lastErr = err;
      const msg = String(err?.message || '');
      const retriable = msg === 'aborted' || /ECONNRESET|socket hang up|ETIMEDOUT|EAI_AGAIN|timeout/i.test(msg);
      if (!retriable || attempt === 2) break;
      await randomDelay(800 + attempt * 600, 1600 + attempt * 900);
    }
  }

  if (!html) {
    console.warn(`[fb-scraper] HTML detail fetch failed for ${listingId}: ${lastErr?.message || 'no response'}`);
    return null;
  }

  try {

    // Extract OG meta tags — always present on public listing pages
    const ogDesc = html.match(/<meta\s+property="og:description"\s+content="([^"]+)"/)?.[1] || '';
    const ogImage = html.match(/<meta\s+property="og:image"\s+content="([^"]+)"/)?.[1] || '';
    const ogTitle = html.match(/<meta\s+property="og:title"\s+content="([^"]+)"/)?.[1] || '';

    // Decode HTML entities in description
    const description = ogDesc
      .replace(/&amp;/g, '&')
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>')
      .replace(/&quot;/g, '"')
      .replace(/&#039;/g, "'")
      .replace(/&#39;/g, "'");

    // Extract photos from the listing_photos JSON array embedded in the page.
    // IMPORTANT: scope to that array only — the full page has 30+ scontent URLs
    // (ads, thumbnails, profile pics) that we must not grab.
    // We deliberately skip og:image because it is always a scaled copy of
    // listing_photos[0], and including it creates a duplicate in the merged list.
    const photos = [];

    const lpKey = '"listing_photos":[';
    const lpStart = html.indexOf(lpKey);
    if (lpStart !== -1) {
      // Find the matching closing bracket by tracking depth
      let depth = 0;
      let i = lpStart + lpKey.length - 1; // points at '['
      const arrayStart = i;
      for (; i < html.length; i++) {
        if (html[i] === '[') depth++;
        else if (html[i] === ']') { depth--; if (depth === 0) break; }
      }
      const section = html.slice(arrayStart, i + 1);
      const uriMatches = [...section.matchAll(/"uri"\s*:\s*"([^"]+)"/g)];
      for (const m of uriMatches) {
        let uri;
        try { uri = JSON.parse('"' + m[1] + '"'); }
        catch { uri = m[1].replace(/\\u0026/g, '&').replace(/\\\//g, '/'); }
        if (uri && !photos.includes(uri)) photos.push(uri);
      }
    }

    // Fall back to og:image only if we found no listing_photos
    if (photos.length === 0 && ogImage) photos.push(ogImage);

    // Try to find creation_time
    const ctMatch = html.match(/"creation_time"\s*:\s*(\d+)/);
    const creationTime = ctMatch ? parseInt(ctMatch[1], 10) : null;

    // Try to find seller name
    const sellerMatch = html.match(/"seller_name"\s*:\s*"([^"]+)"/) ||
                        html.match(/"display_name"\s*:\s*"([^"]+)"/);
    const sellerName = sellerMatch?.[1] || '';

    return {
      // Format compatible with mergeDetail()
      redacted_description: { text: description },
      description,
      listing_photos: photos.map((uri) => ({ image: { uri } })),
      creation_time: creationTime,
      marketplace_listing_seller: sellerName ? { name: sellerName } : null,
      _source: 'html_scrape',
    };
  } catch (err) {
    console.warn(`[fb-scraper] HTML detail parse failed for ${listingId}: ${err.message}`);
    return null;
  }
}

// ─── Normalisation ────────────────────────────────────────────────────────────

/**
 * Merge a detail object's extra fields into a search listing object.
 * Called when we have both the search summary and the full detail.
 *
 * @param {object} searchListing  Normalised listing from search results
 * @param {object} rawDetail      Raw FB detail object from getListingDetail()
 * @returns {object}              Merged listing with all available fields
 */
export function mergeDetail(searchListing, rawDetail) {
  if (!rawDetail) return searchListing;

  // Description — detail page always has the full text
  const description =
    rawDetail.redacted_description?.text ||
    rawDetail.description?.text ||
    rawDetail.description ||
    searchListing.description ||
    '';

  // All photos from detail (authoritative — use exclusively when available)
  const detailPhotos =
    rawDetail.listing_photos ||
    rawDetail.photos ||
    rawDetail.media_photos ||
    [];
  const detailPhotoUris = [];
  for (const p of detailPhotos) {
    const uri = p?.image?.uri || p?.image?.url || p?.uri;
    if (uri && !detailPhotoUris.includes(uri)) detailPhotoUris.push(uri);
  }
  // If detail gave us photos, use only those (avoids duplicates from CDN URL variants).
  // Otherwise fall back to whatever the search result had.
  const photos = detailPhotoUris.length > 0 ? detailPhotoUris : [...searchListing.photos];

  // Seller info — detail page always has name
  const sellerRaw =
    rawDetail.marketplace_listing_seller ||
    rawDetail.seller ||
    rawDetail.author ||
    {};
  const sellerName =
    sellerRaw.name ||
    sellerRaw.display_name ||
    searchListing.seller.name;
  const sellerId = sellerRaw.id || searchListing.seller.id;

  // Creation time
  const postedAt =
    rawDetail.creation_time
      ? new Date(rawDetail.creation_time * 1000).toISOString()
      : rawDetail.listed_time
      ? new Date(rawDetail.listed_time * 1000).toISOString()
      : searchListing.postedAt;

  return {
    ...searchListing,
    description,
    photos,
    seller: { ...searchListing.seller, name: sellerName, id: sellerId },
    postedAt,
    _rawDetail: rawDetail,
  };
}

/**
 * Normalise a raw Facebook listing from a search edge into the shared pipeline format.
 *
 * @param {object} fbListing  Raw listing object from Facebook GraphQL search response
 * @returns {object}          Normalised deal object
 */
export function normalizeListing(fbListing) {
  if (!fbListing) return null;

  // ID — can be in several places
  const id = fbListing.id || fbListing.listing_id || fbListing.target?.id || '';

  // Title
  const title =
    fbListing.marketplace_listing_title ||
    fbListing.listing_title ||
    fbListing.name ||
    fbListing.title ||
    '';

  // Price — search results have listing_price; detail may have it differently
  const priceObj = fbListing.listing_price || fbListing.price;
  const priceCents = parseFBPrice(priceObj);
  const priceAmount = priceCents !== null ? priceCents / 100 : null;
  const currency = priceObj?.currency || priceObj?.amount_with_offset_in_currency?.currency || 'EUR';

  // Location
  const locationData =
    fbListing.location?.reverse_geocode ||
    fbListing.listing_location?.reverse_geocode ||
    fbListing.location ||
    {};
  const city =
    locationData.city ||
    locationData.city_page?.name ||
    locationData.neighborhood ||
    '';
  const state =
    locationData.state ||
    locationData.state_abbreviation ||
    locationData.region ||
    '';

  // Seller — may be null in search results (privacy), available in detail
  const sellerRaw =
    fbListing.marketplace_listing_seller ||
    fbListing.seller ||
    fbListing.author ||
    {};
  const sellerName =
    sellerRaw.name || sellerRaw.display_name || '';
  const sellerId = sellerRaw.id || sellerRaw.marketplace_seller_id || '';

  // Photos
  const photos = [];
  // Primary photo
  const primaryUri =
    fbListing.primary_listing_photo?.image?.uri ||
    fbListing.primary_listing_photo?.image?.url ||
    fbListing.cover_photo?.image?.uri;
  if (primaryUri) photos.push(primaryUri);

  // Additional photos
  const allPhotos =
    fbListing.listing_photos ||
    fbListing.photos ||
    fbListing.media_photos ||
    [];
  for (const p of allPhotos) {
    const uri = p?.image?.uri || p?.image?.url || p?.uri;
    if (uri && !photos.includes(uri)) photos.push(uri);
  }

  // Condition from sub-titles
  let condition = null;
  const subTitles = fbListing.custom_sub_titles_with_rendering_flags || [];
  let shippingOffered = Boolean(
    fbListing.is_shipping_offered ||
    fbListing.shipping_offered ||
    fbListing.marketplace_shipping_eligible ||
    fbListing.marketplace_shipping_seller_eligible ||
    fbListing.shipping_eligible
  );
  let shippingText = '';
  for (const sub of subTitles) {
    const text = sub?.subtitle || sub?.text || '';
    if (!condition && /bueno|buen estado|good|fair|excellent|nuevo|new|usado|used|like new/i.test(text)) {
      condition = text.trim();
    }
    if (!shippingOffered && /ship|shipping|delivery|pickup/i.test(text)) {
      shippingOffered = /ship|shipping|delivery/i.test(text);
      shippingText = text.trim();
    }
  }
  // Also try condition from listing directly
  if (!condition) {
    condition =
      fbListing.condition ||
      fbListing.listing_condition?.display_name ||
      null;
  }

  // Description
  const description =
    fbListing.redacted_description?.text ||
    fbListing.description?.text ||
    fbListing.description ||
    '';

  if (!shippingText) {
    shippingText =
      fbListing.shipping_label?.text ||
      fbListing.shipping_label ||
      fbListing.delivery_type ||
      fbListing.delivery_method ||
      '';
  }

  // Creation time — epoch seconds
  const creationTime =
    fbListing.creation_time ||
    fbListing.listed_time ||
    fbListing.created_time ||
    null;

  return {
    id,
    title,
    price: priceAmount,
    currency,
    description,
    photos,
    seller: {
      name: sellerName || 'Unknown',
      id: sellerId,
      location: [city, state].filter(Boolean).join(', '),
    },
    condition,
    postedAt: creationTime ? new Date(creationTime * 1000).toISOString() : null,
    isPending: fbListing.is_pending || false,
    shippingOffered,
    shippingText,
    url: buildListingUrl(id),
    source: 'facebook',
  };
}
